    ...
    public static void Main() {
        ...
        */
        List<ReportItem> millionaireReport = customers
            ...
            .OrderBy(customer => customer.Name.Split(' ').Last())
            .Select(customer => new ReportItem
            {
                CustomerName = customer.Name,
                BankSymbol = customer.Bank
            })
            .ToList();

        foreach (ReportItem item in millionaireReport)
        {
            string bankName = banks.First(bank => bank.Symbol == item.BankSymbol).Name;
            Console.WriteLine($"{item.CustomerName} at {bankName}");
        }
    }
}

// PREVIOUS EXERCISES IN CHAPTER 15!!! STARTING AT THE TOP WORKING DOWN
